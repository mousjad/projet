...
class DTN(object):
            ...
            with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=None,
                ...
                with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True, 
                    ...
                    if self.mode == 'pretrain':
                        net = slim.conv2d(net, 7606, [1, 1], padding='VALID', scope='out')
    ...
    def build_model(self):
        ...
        if self.mode == 'pretrain':
            self.images = tf.placeholder(tf.float32, [None, 64, 48, 3], 'svhn_images')
            ...
            # source domain (svhn to mnist)32
            self.fx = self.content_extractor(self.images)
            self.sampled_images = self.generator(self.fx)

        elif self.mode == 'train':
            self.src_images = tf.placeholder(tf.float32, [None, 64, 48, 3], 'svhn_images')
            self.trg_images = tf.placeholder(tf.float32, [None, 64, 48, 3], 'mnist_images')
